...
CITY = "belohorizonte"
COURT = "duna"
RTSP_URL = "rtsp://apertaiCam3:130355va@192.168.0.7:554/Streaming/Channels/101"
BUCKET_NAME = "videos-283812"
CREDENTIALS_PATH = "/home/apertai/Desktop/apertaiKeys.json"

def start_buffer_stream_1():
    print(f"Starting cam3 buffer 1 at {datetime.now()}")
    ...
    buffer_command = [
        ...
        'cam-3-buffer-1-%03d.ts-%03d.ts'  # Save segments with a numbering pattern
    ]
    return subprocess.Popen(buffer_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

def start_buffer_stream_2():
    print(f"Starting cam3 buffer 2 at {datetime.now()}")
    ...
    buffer_command = [
        ...
        '-segment_wrap', '1',  # Number of segments to wrap around
        '-reset_timestamps', '1',  # Reset timestamps at the start of each segment
        'cam-3-buffer-2-%03d.ts-%03d.ts'  # Save segments with a numbering pattern
    ]
    return subprocess.Popen(buffer_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
